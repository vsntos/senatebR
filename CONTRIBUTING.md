...
Obrigado por considerar contribuir com o **senatebR**! Este documento
descreve como reportar problemas, sugerir melhorias e submeter código.
...
4.  Versão do R (`R.version.string`) e versão do pacote
    (`packageVersion("senatebR")`)

## Sugerindo novas funções

O pacote cobre dados disponíveis na [API do Senado
Federal](https://legis.senado.leg.br/dadosabertos/docs). Ao sugerir uma
nova função, indique:
...
| Situação                      | Usar                                                                   |
|-------------------------------|------------------------------------------------------------------------|
| Falha de rede / HTTP          | [`warning()`](https://rdrr.io/r/base/warning.html) + `return(NULL)`    |
| Dados não encontrados         | [`message()`](https://rdrr.io/r/base/message.html) (execução continua) |
| Parâmetro de entrada inválido | [`stop()`](https://rdrr.io/r/base/stop.html)                           |

**Testes:** Toda nova função deve ter ao menos um teste em
`tests/testthat/`. Funções que consomem API devem usar fixtures XML
locais (veja `tests/testthat/fixtures/`) ou
[`testthat::local_mocked_bindings()`](https://testthat.r-lib.org/reference/local_mocked_bindings.html)
para evitar chamadas de rede nos testes.
...
Este projeto segue o [Contributor
Covenant](https://www.contributor-covenant.org/). Seja respeitoso e
construtivo nas interações.
