...
extrair_notas_taquigraficas <- function(codigos_reuniao) {
  ...
  for (codigo_reuniao in codigos_reuniao) {
    ...
    tryCatch({
      ...
      if (length(divs) == 0) {
        message("Nenhuma nota taquigr\u00e1fica encontrada para o c\u00f3digo de reuni\u00e3o: ", codigo_reuniao)
      } else {
      ...
      warning("Erro ao acessar a URL: ", url_nota_taquigrafica)
  ...
  if (nrow(df_conteudos) == 0) {
    message("Nenhuma nota taquigr\u00e1fica encontrada para os c\u00f3digos de reuni\u00e3o fornecidos.")
    return(NULL)
  }
